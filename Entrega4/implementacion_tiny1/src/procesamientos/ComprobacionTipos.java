...
public class ComprobacionTipos implements Procesamiento {
	...
	public void procesa(Insts_muchas insts_muchas) {
		...
		if (insts_muchas.insts().tipo == DescTipo.OK && insts_muchas.inst().tipo == DescTipo.OK) {
			insts_muchas.tipo = DescTipo.OK;
		} else {
			insts_muchas.tipo = DescTipo.ERROR;
			error();
		}
	}
	...
	public void procesa(E_igual e_igual) {
		...
		if (e_igual.var().esDesignador() && compatibilidad(e_igual.var().tipo, e_igual.val().tipo)) {
